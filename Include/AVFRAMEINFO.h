...
typedef struct _FRAMEINFO
{
	...
	unsigned char liveFlag;	// 0 - n//liveFlag  1 byte   0实时，1录像

	unsigned char onlineNum;	// number of client connected this device
//	unsigned char reserve1[3];
    unsigned char useCount;//用来确认seek命令的执行情况，根据我发送的seek参数确定
    
    unsigned short frmNo;//包信息，上报时使用 6
    //unsigned char reserve1;//无用信息 7
...
//	unsigned int reserve2;	//
    ...
    unsigned char isday;//判断白天黑夜，1，白天，0：黑夜
    
    unsigned char res[8];//预留，防止增加指令导致buffer过小而不能播放
